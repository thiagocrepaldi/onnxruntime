...
bool IsCropWindowValid(const UncompressFlags& flags, int input_image_width, int input_image_height) {
  // Crop window is valid only if it is non zero and all the window region is
  // within the original image.
  return flags.crop_width > 0 && flags.crop_height > 0 && flags.crop_x >= 0 && flags.crop_y >= 0 &&
         flags.crop_y + flags.crop_height <= input_image_height && flags.crop_x + flags.crop_width <= input_image_width;
}
...
uint8* UncompressLow(const void* srcdata, FewerArgsForCompiler* argball) {
  ...
  if (flags.crop) {
    ...
    if (!IsCropWindowValid(flags, cinfo.output_width, cinfo.output_height)) {
      std::cout << "Invalid crop window: x=" << flags.crop_x << ", y=" << flags.crop_y << ", w=" << target_output_width
    ...
    if (skipped_scanlines != flags.crop_y) throw std::runtime_error("crop error");
  }
#endif
  ...
  if (flags.crop && cinfo.output_scanline < cinfo.output_height) {
    ...
    jpeg_skip_scanlines(&cinfo, cinfo.output_height - flags.crop_y - flags.crop_height);
...
uint8* Uncompress(const void* srcdata, int datasize, const UncompressFlags& flags, int64* nwarn,
  ...
  const float fraction_read = argball.height_ == 0 ? 1.0 : (static_cast<float>(argball.height_read_) / argball.height_);
